    ...
    if not rospy.is_shutdown():  
        ...
        try:
            ...
            if i == 0:
                ...
                # Minimum Z value for the surface
                min_Z = min(points[:, 2])
                MinX = min(points[:, 0])
                MinY = min(points[:, 1])
                MaxX = max(points[:, 0])
                ...
                # Calculate center point of the surface
                ...
                # Create a PointStamped for the center point
                desired_point_stamped = PointStamped()
                ...
                # Ellipse parameters for generating motion
                a = 0.2
                b = 0.25
                ...
                # Smoothly vary the height (z-axis)
                z_start = actual_pose[2]
                z_target = min_Z + 0.05  # Slightly above the surface
                z_steps = np.linspace(z_start, z_target, len(x))

                # Combine x, y, and smoothly interpolated z values
                output = np.column_stack((x, y, z_steps))

                # Function to calculate quaternion for orientation towards the center point from below
                def calculate_orientation_towards(center_point, robot_position):
                    # Compute direction vector from robot to center point
                    ...
                    # Set orientation to look from below the center point
                    z_axis = np.array([0, 0, 1])  # Point upwards
                    x_axis = np.array([1, 0, 0])  # Right direction
                    y_axis = np.cross(z_axis, x_axis)  # Up direction
                    rotation_matrix = np.column_stack((x_axis, y_axis, z_axis))
                    quaternion = tfs.quaternion_from_matrix(rotation_matrix)

                    return quaternion

                # Move the robot smoothly along the path
                j = 0
                for k, desired_point in enumerate(output):
                    # Calculate orientation quaternion to view from below
